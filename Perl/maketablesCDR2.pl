...
my $fout="CDRsumary_thera_map_CA.csv";
        ...
        for (my $j = 1; $j < 6; $j++) {     #each replica of rosetta
            foreach my $top (@toproseta){           # each best X selected of rosetta
                ...
                while(<ROS>)
                {
                    ...
                    print "$pdb - $CDR - $i - $j - $top : $table{$pdb}{$CDR}{$i}{$j}{$top}{R_CA} \n";
                }
...
#        print OUT "$table{$pdb}{$CDR}{seq},$table{$pdb}{$CDR}{lenseq},$table{$pdb}{$CDR}{1}{time},$table{$pdb}{$CDR}{2}{time},$table{$pdb}{$CDR}{3}{time},";
        #print OUT "$table{$pdb}{$CDR}{seq},$table{$pdb}{$CDR}{lenseq},";
        ...
        foreach my $top (@toproseta){
            ...
            for (my $i = 1; $i < 4; $i++) {
                my $p=0;
                for (my $j = 1; $j < 6; $j++) {  
                    print OUT "$table{$pdb}{$CDR}{$i}{$j}{$top}{R_CA},";
                    $p=$p+$table{$pdb}{$CDR}{$i}{$j}{$top}{R_CA};
                    $t=$t+$table{$pdb}{$CDR}{$i}{$j}{$top}{R_CA};
                }
                my $meanp=$p/5;
                print OUT "$meanp,";
            }
    ...
    #printing Ave5
     foreach my $CDR (@names){    
        ...
        for (my $i = 1; $i < 4; $i++) {
            my $p2=0;
            for (my $j = 1; $j < 6; $j++) {  
                print OUT "$table{$pdb}{$CDR}{$i}{$j}{Ave5}{R_CA},";
                $p2=$p2+$table{$pdb}{$CDR}{$i}{$j}{Ave5}{R_CA};
                $t2=$t2+$table{$pdb}{$CDR}{$i}{$j}{Ave5}{R_CA};
            }
            my $meanp2=$p2/5;
            print OUT "$meanp2,";
        }
