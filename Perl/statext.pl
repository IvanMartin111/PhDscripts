...
#Delete the first element of @columns to avoid use it as a number and give an bug output
shift(@columns);
#shift(@sizecol);
#shift(@sizedec);
print "\nColumns: @columns\n" if $debug;
print "Column sizes: @sizecol\n" if $debug;
print "Column sizes: @sizedec\n" if $debug;
...
#	$avg /= $frow - $irow +1;
	...
	printf $format,$avg;
	print " ( $sizecol[$col].$sizedec[$col] )" if $debug;
}
printf("\n");
#printf(" :Averages\n");
#print "@avgs\n";
...
foreach my $col (@columns)
{
	my $sig;
	foreach my $row ($irow .. $frow)
	{ $sig += ($data{$col,$row} - $avgs[$col-$icol-1])**2; } #printf "\ndata %f\tavgs %f\n",$data{$col,$row},$avgs[$col-$icol];}
	$sig /= $N; 
	$sig = sqrt($sig);
	#push(@sigs,$sig);
        $format="%$sizecol[$col].$sizedec[$col]f";
