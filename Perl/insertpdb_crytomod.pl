#!/usr/bin/perl
#
# Insert one PDB structure into another at specified residue indices. 
#
# by Mon (17/11/2014)
#
...
# Loading things        
my $debug = 0;
my $prog = "insertpdb_crytomod";
my $rcdfile = $ARGV[0];
my $sufix= "_modeling_L3_c2m_noloop_min.pdb";
	...
	foreach my $line (@{$completeA}) {
		...
		if ($chain eq "L"){
			if (!($res_num eq $cur_res)){
				$cur_res =$res_num;
				if ($res_num eq $start){$activateloop=1;}
				if ($res_num eq $end2){$activateloop=0;}
			}
		}
