...
my $fout="CDRsumary_AF_map_CB.csv";

my $H1_rcd="20K_H1_AF_map_rep";
my $H2_rcd="20K_H2_AF_map_rep";
my $H3_rcd="50K_H3_AF_map_rep";
my $L1_rcd="20K_L1_AF_map_rep";
my $L2_rcd="20K_L2_AF_map_rep";
my $L3_rcd="20K_L3_AF_map_rep";


$rosnam{H1}="_modeling_20K_H1_AF_map_rep";  #rosettaX  _native_1K_H1_map_rep  X.txt
$rosnam{H2}="_modeling_20K_H2_AF_map_rep";  
$rosnam{H3}="_modeling_50K_H3_AF_map_rep";  
$rosnam{L1}="_modeling_20K_L1_AF_map_rep"; 
$rosnam{L2}="_modeling_20K_L2_AF_map_rep";  
$rosnam{L3}="_modeling_20K_L3_AF_map_rep"; 
    ...
    for (my $i = 1; $i < 4; $i++) {     #each replica con RDC
       ...
       close (RES); 
        ...
        foreach my $top (@toproseta){           # each best X selected of rosetta
            ...
            my $RS=$rcd.$i."/rosettaRS".$top.".txt";

            open (RS,$RS) or die "The file $RS doesn't exit in this path.\n";
            while(<RS>)
                {
                next if /^#/; # reading only non-# beginning lines
                $_=~ s/^\t//;
                ...
                my $pdb = substr($line[1],0,4);
                $table{$pdb}{$CDR}{$i}{$top}{RS}{E_full2} = $line[10];
                $table{$pdb}{$CDR}{$i}{$top}{RS}{R_CA} = $line[11];
                $table{$pdb}{$CDR}{$i}{$top}{RS}{R_BB} = $line[12];
                $table{$pdb}{$CDR}{$i}{$top}{RS}{R_BBO} = $line[13];
                $table{$pdb}{$CDR}{$i}{$top}{RS}{R_CB} = $line[14];
                $table{$pdb}{$CDR}{$i}{$top}{RS}{R_OCB} = $line[15];
            }
            close(RS);
...
foreach my $CDR (@names){
     foreach my $ros(@toproseta){

        for (my $i = 1; $i < 4; $i++) {
             print OUT $CDR."_Top".$ros."-RS:$i,";
        }
        print OUT $CDR."_Top".$ros."-RS: T-mean,";
    }
}
print OUT "\n";
...
foreach my $pdb (sort keys %table){
    ...
    foreach my $CDR (@names){
         ...
         my $t2=0;
            for (my $i = 1; $i < 4; $i++) {
                 print OUT "$table{$pdb}{$CDR}{$i}{$top}{RS}{R_CB},";
                 $t2=$t2+$table{$pdb}{$CDR}{$i}{$top}{RS}{R_CB};
            }
            my $meant2=$t2/3;
            print OUT "$meant2,";
        }
