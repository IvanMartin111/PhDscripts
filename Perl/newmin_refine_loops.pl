...
{
	printf "$prog> NATIVE SCENARIO. PROCESSING loop %3d: @{$loop}\n",$nloop+1;
	my $pdb = $loop->[0];
	my $start = $loop->[1];
	my $end = $loop->[2];
	my $chain = $loop->[3];
...
my $kk = "head -n 1 $base$outsuff"."_rosetta.txt > rosetta1$outsuff.txt; for i in *$outsuff"."_rosettaS.txt; do var=\`awk \'NR==3\' \$i\ | sort -n -k $col_rmsd | head -n 1`; echo -e \"\${i:0:20} \$var\"; done > temp; statext.pl temp 1 17 1 all 2 >> rosetta1$outsuff.txt; rm temp";
print "EXECUTING: $kk\n";
open my $fh, '-|', qw(/bin/bash -c), $kk;
close($fh);
# Get best-2 energy decoys statistics
$kk = "head -n 1 $base$outsuff"."_rosetta.txt > rosetta2$outsuff.txt; for i in *$outsuff"."_rosettaS.txt; do var=\`awk \'NR>=3 && NR<3+2\' \$i\ | sort -n -k $col_rmsd | head -n 1`; echo -e \"\${i:0:20} \$var\"; done > temp; statext.pl temp 1 17 1 all 2 >> rosetta2$outsuff.txt; rm temp";
print "EXECUTING: $kk\n";
open my $fh, '-|', qw(/bin/bash -c), $kk;
close($fh);
# Get best-5 energy decoys statistics
$kk = "head -n 1 $base$outsuff"."_rosetta.txt > rosetta5$outsuff.txt; for i in *$outsuff"."_rosettaS.txt; do var=\`awk \'NR>=3 && NR<3+5\' \$i\ | sort -n -k $col_rmsd | head -n 1`; echo -e \"\${i:0:20} \$var\"; done > temp; statext.pl temp 1 17 1 all 2 >> rosetta5$outsuff.txt; rm temp";
print "EXECUTING: $kk\n";
open my $fh, '-|', qw(/bin/bash -c), $kk;
close($fh);
# Get best-10 energy decoys statistics
$kk = "head -n 1 $base$outsuff"."_rosetta.txt > rosetta10$outsuff.txt; for i in *$outsuff"."_rosettaS.txt; do var=\`awk \'NR>=3 && NR<3+10\' \$i\ | sort -n -k $col_rmsd | head -n 1`; echo -e \"\${i:0:20} \$var\"; done > temp; statext.pl temp 1 17 1 all 2 >> rosetta10$outsuff.txt; rm temp";
print "EXECUTING: $kk\n";
open my $fh, '-|', qw(/bin/bash -c), $kk;
close($fh);
# Get best-20 energy decoys statistics
$kk = "head -n 1 $base$outsuff"."_rosetta.txt > rosetta20$outsuff.txt; for i in *$outsuff"."_rosettaS.txt; do var=\`awk \'NR>=3 && NR<3+20\' \$i\ | sort -n -k $col_rmsd | head -n 1`; echo -e \"\${i:0:20} \$var\"; done > temp; statext.pl temp 1 17 1 all 2 >> rosetta20$outsuff.txt; rm temp";
print "EXECUTING: $kk\n";
open my $fh, '-|', qw(/bin/bash -c), $kk;
close($fh);
